...
class Team2 extends Team {
  ...
  Team2(int team_id, int tank_size, color c, 
    ...
    tanks[0] = new Tank5(tank0_id, this, this.tank0_startpos, this.tank_size, ball0);
  ...
  public class Tank4 extends Tank {
     ...
     GeneralProblem<PVector, String> problem; // Structure to formulate problem
     ...
     Tank4(int id, Team team, PVector startpos, float diameter, CannonBall ball) {
       super(id, team, startpos, diameter, ball);
       started = false;
       reporting = false;
    ...
    */
    ...
    void formulateGoal() {
      ...
      problem = new GeneralProblem(position, aFunc, rFunc, gPred);
      println("goal" + goal);
    }
    /*
    * Action "up"
    ...
    * Action "left"
    *
    * Moves the agents position x-50 (Easst on the grid)
    *
    */
    void moveLeft() {
      if(!isOutOfBounds(position.x-50)) {
    ...
    // Initial wandering function
    // Not used
    void wander() {
      ArrayList<String> arrayList = getPossibleActions();
      Random rand = new Random();
      int inte = rand.nextInt(arrayList.size());
      String action = arrayList.get(inte);
    ...
    */
    void lrta() {
      if(!started)
        return;
      String action = search.LRTA(position, problem);
      if(action.equalsIgnoreCase("up")) {
        moveUp();
      } else if (action.equalsIgnoreCase("down")) {
        moveDown();
    ...
    // Action function to get possible action through sensor.
    // Not used
    Function<PVector, List<String>> test = (state) -> {
  ArrayList<String> actions = new ArrayList<>();
      PVector obs = front_sensor.readValue1();
      println(Math.round(position.x));
      println(Math.round(obs.x));
      println(Math.round(obs.y));
      println(Math.round(position.y));
      println(degrees(this.heading));
      if(Math.round(position.x) == Math.round(obs.x) && Math.round(obs.y) > Math.round(position.y) && search.manhattanDistance(position, obs) > 100) {
        actions.add("up");
      }
      if(Math.round(position.x) == Math.round(obs.x) && Math.round(obs.y) < Math.round(position.y) && search.manhattanDistance(position, obs) > 100) {
        actions.add("down");
      }
      if(Math.round(position.x) > Math.round(obs.x) && Math.round(position.y) == Math.round(obs.y) && search.manhattanDistance(position, obs) > 100) {
        actions.add("left");
      }
      if(Math.round(position.x) < Math.round(obs.x) && Math.round(position.y) == Math.round(obs.y) && search.manhattanDistance(position, obs) > 100) {
        actions.add("right");
      } 
      if (Math.round(position.x) == Math.round(obs.x) && (Math.round(position.y) - Math.round(obs.y)) < 100) {
  ...
  * Action function that simulates percepts by asking the environment about the content of closeby nodes
  ...
  */
  ...
  Function<PVector, List<String>> sens = (state) -> {
    ArrayList<String> actions = new ArrayList<>();
    if(grid.getNearestNode(new PVector(state.x, state.y-50)).empty()) {
        actions.add("up");
      }
      if(grid.getNearestNode(new PVector(state.x, state.y+50)).empty()) {
        actions.add("down");
      }
      if(grid.getNearestNode(new PVector(state.x+50, state.y)).empty()) {
        actions.add("right");
      }
      if(grid.getNearestNode(new PVector(state.x-50, state.y)).empty()) {
        actions.add("left");
    ...
    public void updateLogic() {
      ...
      if (!started) {
        ...
        println(problem.getGoal());
        if(!problem.testGoal(goal)) {
          println("wohoo");
        }
        // ASTAR or offline search algorithms (Uncomment when trying a star)
