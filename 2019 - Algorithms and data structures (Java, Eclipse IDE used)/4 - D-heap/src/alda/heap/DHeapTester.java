	...
	public void testFunctionality() {
		...
		for (int n = 0; n < 1000; n++) {
			...
			while (!heap.isEmpty() && rnd.nextBoolean()) {
				System.out.println("something of this is getting deleted");
				for (int d = 0; d < heap.size(); d++) {
					System.out.println(heap.get(d ));
				}
				System.out.println("This is the minimun deleted:");
	            System.out.println(heap.findMin());
	            System.out.println(heap.firstChildIndex(1));
				assertEquals(oracle.poll(), heap.deleteMin());
				System.out.println("AFFFFFFFTER:");
				for (int d = 0; d < heap.size(); d++) {
					System.out.println(heap.get(d ));
				}
			}
...
//	}
	...
	private void testValues(Integer... expected) {
		...
		for (int n = 0; n < expected.length; n++) {
			System.out.println(heap.get(n + 1));
		}
		for (int n = 0; n < expected.length; n++) {
