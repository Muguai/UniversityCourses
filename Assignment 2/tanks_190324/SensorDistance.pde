...
class SensorDistance extends Sensor{
  ...
  public PVector readValue1(){
    ...
    println("this is tank heading damnit " + tank.heading);
    if(tank.heading > 2.5) {
      pvec = new PVector(0, tank.position.y);
    } if (tank.heading > 1.707964 && tank.heading < 3) {
      pvec = new PVector(tank.position.x, 800);
    } if (tank.heading < 1 && tank.heading > -1) {
      pvec = new PVector(800, tank.position.y);
    } if (tank.heading < -1.507964) {
      pvec = new PVector(tank.position.x, 0);
    }
    println("din mamma" + pvec.x + " " + pvec.y);
    return pvec;
  }
  
   //******************************************************
  // Returnera avståndet till kanten i tankens riktning.
  public PVector readValue2(){
