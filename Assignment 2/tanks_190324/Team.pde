...
class Team {
    ...
    PVector tank2_startpos, int tank2_id, CannonBall ball2) 
  {
    ...
    if (this.id==0) {
      this.homebase_x = 0; this.homebase_y = 0;
      this.enemybase_x = width - 151; this.enemybase_y = height - 351;
    }
    else if (this.id==1) {
      this.homebase_x = width - 151; this.homebase_y = height - 351;
      this.enemybase_x = 0; this.enemybase_y = 0;
    }
  }
