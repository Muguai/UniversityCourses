...
class Tank extends Sprite {
  ...
  //FOR CBS
  GeneralProblem<PVector, String> generalProblem;
  PVector goal;
  Queue<Nod> pathQueue;
  Search search;
  ...
  Tank(int id, Team team, PVector _startpos, float diameter, CannonBall ball) {
    ...
    // Set startnode content
    grid.getNearestNode(startpos).addContent(this);
  ...
  protected void initializeSensors() {
   ...
   //  SensorCompass compass = new SensorCompass(game, this);
     ...
     SensorDistance ultrasonic_left = new SensorDistance(this, 270f);
     registerSensor(ultrasonic_left, "ULTRASONIC_LEFT");
     
     SensorDistance ultrasonic_right = new SensorDistance(this, 90f);
  ...
  void checkEnvironment() {
    ...
    if (
      position.x > team.enemybase_x && 
      position.x < team.enemybase_x+team.enemybase_width &&
      position.y > team.enemybase_y &&
      position.y < team.enemybase_y+team.enemybase_height) {
      if (!isAtEnemybase) {
        isAtEnemybase = true;
        message_arrivedAtEnemybase();
      }
    } else {
      isAtEnemybase = false;
    }
  ...
  // Tanken meddelas om att tanken är i hembasen.
  public void message_arrivedAtEnemybase() {
    println("! Tank["+ this.getId() + "] – har kommit till fiendebasen.");
  }
